			      ...
			      struct spl_boot_device *bootdev)
{
	...
	printf("spi_load: load_offset=0x%08x\n", load_offset);
	spi0_init();

	ret = spi0_read_data((void *)header, load_offset, 0x40);
	if (ret)
		goto out;
	printf("spi_load: image header read, magic=0x%08x\n",
	...
	if (IS_ENABLED(CONFIG_SPL_LOAD_FIT) &&
		...
		printf("spi_load: legacy image\n");
		ret = spl_parse_image_header(spl_image, bootdev, header);
		if (ret)
			goto out;

		printf("spi_load: image header parsed, load_addr=0x%08x, size=0x%08x\n",
		       (uint32_t)spl_image->load_addr, spl_image->size);
